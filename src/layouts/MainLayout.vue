<template>
  <q-layout view="lHh Lpr lFf">
    ...
    <q-dialog maximized v-model="showTipDialog">
      <div>
        <close-button
          :top="10"
          style="z-index: 2;"
    ...
    >
      <template v-if="user">
        <router-link to="/Home" class="center-content-horizontal">
      ...
      <template v-else>
        <p class="text-secondary q-pl-sm">
          Du bist nicht Angemeldet! Um deine Reise zu speichern musst du dich
          später links oben Registrieren
        </p>
      </template>
    </q-footer>
    <CookieBanner v-if="isSPA"></CookieBanner>
    <q-dialog maximized v-model="showUpdateDialog" persistent>
...
export default {
  ...
  meta() {
    return {
      title: "Roadtrip planen",
      titleTemplate: title =>
        `${title} | Reise, Route online planen | Routenplaner mit Karte`,
      meta: {
        equiv: {
